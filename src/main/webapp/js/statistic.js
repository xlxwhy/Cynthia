...
function changeTips()
{
	...
	if(statType === "task"){
		content = '1.该统计类型统计任务关联的问题状态趋势变化;<br/>2.开始时间和时间类型必选其一;<br/>3.可选择某一任务统计,也可设置过滤条件统计;<br/>4.必须设置统计的bug字段,该字段关联着问题数据';
	}else if(statType === "person"){
		content = '1.该统计类型统计相应角色解决bug或问题的数量;<br/>2.定义的解决动作为触发该动作即认为解决该问题;<br/>3.包含当前指派数量即是当前指派给该角色的数据;<br/>4.必须设置统计的bug字段,该字段关联着问题数据';
	}else if(statType === "model"){
